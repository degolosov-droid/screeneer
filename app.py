...
def check_filters(ticker, info, hist, min_price, min_market_cap, min_volume, min_adr):
    """Проверяет акцию по первичным фильтрам"""
    try:
        # 1. Цена акции > $min_price
        price = info.get('currentPrice') or info.get('regularMarketPrice')
        if not price or price < min_price:
            return False, f"Цена ${price:.2f} < ${min_price}"
        
        # 2. Капитализация > $min_market_cap M
        market_cap = info.get('marketCap')
        if not market_cap or market_cap < min_market_cap * 1_000_000:
            return False, f"Кап ${market_cap/1_000_000:.0f}M < ${min_market_cap}M"
        
        # 3. Средний объём > min_volume K
        avg_volume = info.get('averageVolume') or info.get('averageVolume10days')
        if not avg_volume or avg_volume < min_volume * 1000:
            return False, f"Объём {avg_volume/1000:.0f}K < {min_volume}K"
        
        # 4. ADR (Average Daily Range) > min_adr%
        ...
        if adr < min_adr:
            return False, f"ADR {adr:.1f}% < {min_adr}%"
...
# Ключ для отслеживания изменений фильтров
filter_key = f"{min_price}_{min_market_cap}_{min_volume}_{min_adr}_{period_key}"

if 'last_filter_key' not in st.session_state:
    st.session_state.last_filter_key = ""

# Кнопка запуска фильтрации или изменение фильтров
if st.button("🚀 Применить фильтры и построить список") or filter_key != st.session_state.last_filter_key:
    # Сбросим кэш перед новым запуском
    st.session_state.filtered_tickers = None
    st.session_state.momentum_data = None
    ...
    with st.spinner("Фильтрация акций..."):
        ...
        for i, ticker in enumerate(EXTENDED_TICKERS):
            try:
                ...
                passed, reason = check_filters(ticker, info, hist, min_price, min_market_cap, min_volume, min_adr)
